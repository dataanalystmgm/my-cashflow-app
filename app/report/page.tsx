...
export default function ReportPage() {
  ...
  const [budgets, setBudgets] = useState<any[]>([]); // State baru untuk budget
  ...
  useEffect(() => {
    ...
    // 2. Ambil Data Budget (Bulan Berjalan)
    ...
    const unsubBudget = onSnapshot(qBudget, (s) => setBudgets(s.docs.map(d => ({ id: d.id, ...d.data() }))));
  ...
  // Kalkulasi data untuk Pie Chart
  ...
  // Gabungkan Data Budget dengan Realisasi (Pengeluaran)
  const budgetAnalysis = budgets.map(b => {
    const spent = expenseData.find((e: any) => e.name === b.categoryName)?.value || 0;
    return {
  ...
  return (
    <main className="max-w-2xl mx-auto p-4 space-y-6 min-h-screen bg-gray-50 pb-20">
      <div className="flex items-center justify-between">
        ...
        {/* Tombol ke Pengaturan Budget/Kategori jika diperlukan */}
        <Link href="/settings" className="text-xs bg-blue-100 text-blue-600 px-3 py-1 rounded-full font-bold">Setting Budget</Link>
      </div>
      ...
      {/* ANALISIS BUDGET VS REALISASI */}
      <Card>
        <h3 className="font-bold text-gray-700 mb-4">Monitor Budget Bulan Ini</h3>
        {budgetAnalysis.length > 0 ? (
          <div className="space-y-4">
            {budgetAnalysis.map((item, i) => (
              <div key={i}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="font-medium text-gray-600">{item.category}</span>
                  <span className={`font-bold ${item.actual > item.limit ? 'text-red-500' : 'text-gray-500'}`}>
                    Rp{item.actual.toLocaleString()} / Rp{item.limit.toLocaleString()}
                  </span>
                </div>
                <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                  <div 
                    className={`h-full transition-all duration-500 ${item.actual > item.limit ? 'bg-red-500' : 'bg-blue-500'}`}
                    style={{ width: `${Math.min(item.percent, 100)}%` }}
                  />
                </div>
                {item.actual > item.limit && (
                  <p className="text-[10px] text-red-500 mt-1 italic">* Melebihi budget!</p>
                )}
              </div>
            ))}
          </div>
        ) : (
          <p className="text-center text-gray-400 py-4 text-sm">Belum ada budget yang diatur bulan ini.</p>
        )}
      </Card>
